...
public class CertService implements AbstractCertService{
    ...
    public boolean addCert(Certification certification, String goalOwnerEmail) {
        Goal goal = goalRepository.selectGoalByGoalId(certification.getGoalId()).orElseThrow(()->new RuntimeException("No matched goal"));
        if(!goal.getMemberEmail().equals(goalOwnerEmail)){
            return false;
        }
        try {
